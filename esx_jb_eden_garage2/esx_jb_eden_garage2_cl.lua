...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Tire o carro" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Remove o carro" , value = 'rename_vehicle'})
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Seu veículo está nos apreendidos')
                        elseif (data.current.value.state) then
                            menu.close()
                            menu2.close()
                            SpawnVehicle(data.current.value.vehicle, garage, KindOfVehicle)
                        else
                            TriggerEvent('esx:showNotification', 'Seu veículo já está fora')
                        end
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Seu carro está na garagem')
					else
		...
		TriggerEvent('esx:showNotification', 'não há veículo para entrar')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'O carro está de volta na garagem')
					else
						TriggerEvent('esx:showNotification', 'Você não pode armazenar este carro na garagem')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'O veículo está de volta na garagem')
					else
						TriggerEvent('esx:showNotification', 'Você não pode armazenar este veículo na garagem')
					end
				end,vehicleProps)
			end
		else
...
function ReturnVehicleMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			...
			table.insert(elements, {label ="Nenhum veículo para sair" , value = nil})
		end
		...
		function(data, menu)
			if data.current.value == 'fourrieremecano' then
				ESX.ShowNotification("Verifique com a polícia ou mecanico para descobrir como recuperar seu carro.")
			elseif data.current.value ~= nil then
				...
				if not iscaronearth then
					...
					ESX.ShowNotification("Você não pode tirar este veículo. Vá buscá-la!")
				end				
			end
		end,
		function(data, menu)
